    ...
    <div class="bg-secondary shadow rounded-2 mb-3 p-3">
        <div class="d-block d-md-flex justify-content-md-evenly">
            <div class="text-start text-md-center mb-2 mb-md-0">
                <span class="d-inline d-md-block text-muted fs-sm">{{ Str::plural('Node', $nodes['total'] ?? 0) }}</span>
                <span class="float-end float-md-none">{{ $nodes['total'] ?? 0 }}</span>
            </div>
            <div class="text-start text-md-center mb-2 mb-md-0">
                <span class="d-inline d-md-block text-muted fs-sm">{{ Str::plural('City', $nodes['cities'] ?? 0) }}</span>
                <span class="float-end float-md-none">{{ $nodes['cities'] ?? 0 }}</span>
            </div>
            <div class="text-start text-md-center">
                <span class="d-inline d-md-block text-muted fs-sm">{{ Str::plural('Country', $nodes['countries'] ?? 0) }}</span>
