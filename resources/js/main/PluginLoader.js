...
export default class PluginLoader {
     ...
     */
    constructor(name, zenonHub, instance) {
        this.name = name;
        this.zenonHub = new Proxy(
            zenonHub,
            InnerProxyHandler,
        );
    ...
    getInstance(...parameters) {
        ...
        if (!this.dependenciesFulfilled()) {
            const unmet = this.getDependencies().filter((item) => !this.zenonHub.getPluginNames().includes(item));
        ...
        const newInstance = new this.instance(this.zenonHub, ...parameters);
    ...
    dependenciesFulfilled() {
        ...
        dependencies.forEach((plugin) => {
            if (!this.zenonHub.hasPlugin(plugin)) {
                fulfilled = false;
            }
        });
