        ...
        <div class="list-group shadow">
            @foreach ($this->user->tokens->sortBy('name') as $token)
                <div class="list-group-item d-flex align-items-baseline">
                    <div class="flex-fill">
                        <span class="d-block text-sm text-heading fw-semibold">{{ $token->name }} {!! collect($token->abilities)->filter(fn($ability) => $ability !== '*')->map(fn($ability) => sprintf('<span class="badge badge-sm text-bg-light opacity-75 ms-2">%s</span>', $ability))->implode(' ') !!}</span>
